...
export default function AppInput(props: AppInputProps) {
    const {value, isDisabled, name, placeholder, isRequired, isFocused, label, className} = props;
    return <div className="space-y-2 flex flex-col">
        ...
        <input type="text" id={name} className={`bg-gray-200 p-3 rounded-2xl md:max-w-72 ${className}`}
               autoFocus={isFocused}
               name={name}
               placeholder={placeholder} defaultValue={value} disabled={isDisabled} required={isRequired}/>
    </div>
}